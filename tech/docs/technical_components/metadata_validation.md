...
### Overview and Scope
In terms of metadata, SoilWise Catalogue aims for an approach that balances harvesting between quantity and quality. See for more information in the [Harvester & Harmonization Component](ingestion.md). Catalogues which capture metadata authored by various data communities typically have a wide range of metadata completeness and accuracy. Therefore, the SoilWise Catalogue employs metadata validation mechanisms to provide additional information about metadata completeness, conformance and integrity. Information resulting from the validation process is stored together with each metadata record in a relational database and updated whenever a new metadata version is registered. Within the first iteration, they are not displayed in the [SoilWise Catalogue](catalogue.md).
